...
vector<Point> GrahamMethod(vector<Point>& VertexVector)
{
	...
	for (size_t i = 1; i < VertexVector.size(); ++i)
	{
		...
		{
			while (up.size() >= 2 && !cw(up[up.size() - 2], up[up.size() - 1], VertexVector[i]))
				up.pop_back();
			up.push_back(VertexVector[i]);
		}
		if (i == VertexVector.size() - 1 || ccw(p1, VertexVector[i], p2))
		{
			while (down.size() >= 2 && !ccw(down[down.size() - 2], down[down.size() - 1], VertexVector[i]))
				down.pop_back();
...
vector<Point> parallelGrahamMethod(const vector<Point>& VertexVector)
{
	...
	MPI_Scatter(VertexVector.data(), delta, structPoint, localVertexVector.data(), delta, structPoint, 0, MPI_COMM_WORLD);
	localGrahamMethod = GrahamMethod(localVertexVector);
	if (ProcRank != 0)
	{
		MPI_Send(localGrahamMethod.data(), localGrahamMethod.size(), structPoint, 0, 0, MPI_COMM_WORLD);
	}
	else {
		for (int i = 1; i < ProcNum; i++)
		{
			...
			MPI_Recv(localGrahamMethod.data() + oldSize, sendElements, structPoint, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		if (ProcNum != 1)
		{
			int tail = VertexVector.size() - ProcNum * delta;
			if (tail) {
				localVertexVector = vector<Point>(VertexVector.begin() + (VertexVector.size() - tail), VertexVector.end());
				globalGrahamMethod = GrahamMethod(localGrahamMethod);

				vector<int>::size_type oldSize = localGrahamMethod.size();
				localGrahamMethod.resize(oldSize + globalGrahamMethod.size());
				for (vector<int>::size_type i = oldSize, j = 0; i < localGrahamMethod.size(); i++, j++)
				{
