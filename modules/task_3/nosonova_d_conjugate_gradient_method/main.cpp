...
TEST(Conjugate_Gradient_Method, simple_matrix) {
    ...
    if (rank == 0) {
        ...
        for (size_t i = 0; i < x.size(); ++i) ASSERT_NEAR(x[i], w[i], 0.000001);
    }
}

TEST(Conjugate_Gradient_Method, big_numbers_test1) {
    int size = 10, rank;
    double** a = nullptr;
    double* b = nullptr;
    std::vector<double> x;
    ...
    if (rank == 0) {
        ...
        for (size_t i = 0; i < x.size(); ++i) ASSERT_NEAR(x[i], w[i], 0.001);
    }
}
TEST(Conjugate_Gradient_Method, big_numbers_test2) {
    int size = 10, rank;
    double** a = nullptr;
    double* b = nullptr;
    std::vector<double> x;
...
TEST(Conjugate_Gradient_Method, big_size_test) {
    ...
    if (rank == 0) {
        ...
        for (size_t i = 0; i < x.size(); ++i) ASSERT_NEAR(x[i], w[i], 0.05);
    }
}
