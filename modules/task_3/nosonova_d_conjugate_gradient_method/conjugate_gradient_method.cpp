...
#include "../../../modules/task_3/nosonova_d_conjugate_gradient_method/conjugate_gradient_method.h"
...
double** getRandomMatrix(int size, double miv, double mav, unsigned seed) {
    ...
    for (int i = 1; i < size; ++i) a[i] = a[i - 1] + size;
    std::mt19937 mt(seed);
    std::uniform_real_distribution<double> urd(miv, mav);
    for (int i = 0; i < size; ++i)
        for (int j = i; j < size; ++j) a[i][j] = a[j][i] = urd(mt);
    return a;
}
...
double* getRandomVector(int size, double miv, double mav, unsigned seed) {
    ...
    std::mt19937 mt(seed);
    std::uniform_real_distribution<double> urd(miv, mav);
    for (int i = 0; i < size; ++i) a[i] = urd(mt);
    return a;
}
...
std::vector<double> conjugateGradientMethod(double** a, const double* b,
    ...
    int rows = size / comm_size +
               ((comm_size - comm_rank <= size % comm_size) ? 1 : 0);

    std::function<double(const std::vector<double>&,
                         const std::vector<double>&)>
        vector_mul = [](const std::vector<double>& a,
    ...
    std::function<void(double**, const std::vector<double>&,
                       std::vector<double>*)>
        mat_vec_mul = [rows](double** a, const std::vector<double>& b,
                             std::vector<double>* r) -> void {
    ...
    for (int i = 1; i < rows; ++i) m[i] = m[i - 1] + size;
    ...
    if (comm_rank == 0) {
        ...
        for (int i = 1; i < comm_size; ++i) {
            int step = size / comm_size +
                       ((comm_size - i <= size % comm_size) ? 1 : 0);
            counts[i] = step * size;
            displs[i] = displs[i - 1] + counts[i - 1];
        }
        MPI_Scatterv(a[0], counts.data(), displs.data(), MPI_DOUBLE, m[0],
                     counts[0], MPI_DOUBLE, 0, MPI_COMM_WORLD);
        for (int i = 0; i < size; ++i) z_all[i] = b[i];
    } else {
        MPI_Scatterv(nullptr, nullptr, nullptr, MPI_DOUBLE, m[0], rows * size,
    ...
    for (int i = 1; i < comm_size; ++i) {
        counts[i] =
            size / comm_size + ((comm_size - i <= size % comm_size) ? 1 : 0);
        displs[i] = displs[i - 1] + counts[i - 1];
    }

    for (int i = 0; i < rows; ++i) z[i] = r[i] = z_all[i + displs[comm_rank]];
    ...
    for (int i = 0; i < size; ++i) {
        ...
        MPI_Allgatherv(z.data(), rows, MPI_DOUBLE, z_all.data(), counts.data(),
    ...
    if (comm_rank == 0) {
        ans.resize(size);
        MPI_Gatherv(x.data(), rows, MPI_DOUBLE, ans.data(), counts.data(),
                    displs.data(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
    } else {
        MPI_Gatherv(x.data(), rows, MPI_DOUBLE, nullptr, nullptr, nullptr,
                    MPI_DOUBLE, 0, MPI_COMM_WORLD);
    }
...
std::vector<double> conjugateGradientMethodOneProc(double** a, const double* b,
    ...
    std::function<void(double**, const std::vector<double>&,
                       std::vector<double>*)>
        mat_vec_mul = [size](double** a, const std::vector<double>& b,
                             std::vector<double>* r) -> void {
        for (int i = 0; i < size; ++i) {
            (*r)[i] = 0.0;
            for (size_t j = 0; j < b.size(); ++j) {
    ...
    for (int i = 0; i < size; ++i) z[i] = r[i] = b[i];
