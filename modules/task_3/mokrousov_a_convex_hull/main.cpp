...
TEST(Convex_Hull_Graham_mpi, test_on_one_point) {
    ...
    for (int i = 0; i < 1; i++) mas[i] = new double[2];
    mas[0][0] = 1;
    mas[0][1] = 1;
    ...
    if (rank == 0) {
        for (int i = 0; i < 1; i++) {
            ...
            for (int j = 0; j < 2; j++) EXPECT_EQ(tmp[j], tmp2[j]);
            res.pop();
            result.pop();
        }
    }
...
TEST(Convex_Hull_Graham_mpi, test_on_my_points) {
    ...
    for (int i = 0; i < 10; i++) mas[i] = new double[2];
    mas[0][0] = -2;
    mas[0][1] = 1;
    mas[1][0] = -1;
    mas[1][1] = 3;
...
TEST(Convex_Hull_Graham_mpi, test_on_one_line) {
    ...
    for (int i = 0; i < 12; i++) mas[i] = new double[2];
    mas[0][0] = -1;
    mas[0][1] = 0;
    mas[1][0] = 3;
    mas[1][1] = 0;
...
TEST(Convex_Hull_Graham_mpi, test_on_my_random_points) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (int j = 0; j < 2; j++) newmas[i][j] = mas[i][j];
    }
