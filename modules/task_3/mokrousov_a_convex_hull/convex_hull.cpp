...
#include "../../../modules/task_3/mokrousov_a_convex_hull/convex_hull.h"
...
double** getRandomMas(int count) {
    ...
    for (int i = 0; i < count; i++) mas[i] = new double[2];
    std::mt19937 seed;
...
double** QuickSort(double** mas, int first, int last, double* fi, double* r) {
    ...
    mid[1] = r[(f + l) / 2];
    do {
        while (SravnenieLess(f, mid, fi, r)) f++;
        while (SravnenieMore(l, mid, fi, r)) l--;
    ...
    if (first < l) QuickSort(mas, first, l, fi, r);
    if (f < last) QuickSort(mas, f, last, fi, r);
    return mas;
}
...
std::stack<double*> ConvSort(double** mas, const int count) {
    ...
    if (count == 1) {
        ...
        for (int i = 1; i < count; i++)
            for (int j = 0; j < 2; j++) mas[i][j] = mas[i][j] - c[j];

        if (rank == 0) {
            for (int i = 1; i < delta + ost; i++) {
                r[i] =
                    pow((mas[i][0] * mas[i][0]) + (mas[i][1] * mas[i][1]), 0.5);
                fi[i] = atan(mas[i][1] / mas[i][0]);
            }
        } else {
            if (f == 0) {
                for (int i = delta * rank + ost; i < delta * rank + delta + ost;
                     i++) {
                    r[i] = pow(
                        (mas[i][0] * mas[i][0]) + (mas[i][1] * mas[i][1]), 0.5);
                    fi[i] = atan(mas[i][1] / mas[i][0]);
                }
            }
        }
        ...
        for (int i = 1; i < size; i++) {
            MPI_Bcast(&fi[delta * i + ost], delta, MPI_DOUBLE, i,
                      MPI_COMM_WORLD);
            MPI_Bcast(&r[delta * i + ost], delta, MPI_DOUBLE, i,
                      MPI_COMM_WORLD);
        }
        ...
        if (rank == 0) {
            ...
            if (f == 0) {
                ...
                for (int i = 1; i < size; i++) {
                    ...
                    MPI_Recv(&buf[0], 2 * s, MPI_DOUBLE, i, 9, MPI_COMM_WORLD,
                             &status);
                    double* last;
                    double* beforelast;
                    int k = 1;
            ...
            if (f == 0) {
                ...
                for (int i = delta * rank + ost + 2;
                     i < delta * rank + ost + delta; i++) {
                    double* last;
                    double* beforelast;
                    while (1) {
                        last = prom_res.top();
