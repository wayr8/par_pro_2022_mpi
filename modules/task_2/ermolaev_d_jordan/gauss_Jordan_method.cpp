...
#include <algorithm>
#include <iostream>
#include "../../../modules/task_2/ermolaev_d_jordan/gauss_Jordan_method.h"
...
double* getParallelGausJordan(double* matrix, int x) {
    ...
    if (rank == root) {
        ...
        num_parts != 0 ? bufftosend = part_size + 1: bufftosend = part_size;

        matrix_sendcounts[0] = bufftosend * (x + 1);

        for (int i = 1; i < col_proc; i++) {
            if (i < num_parts)
                bufftosend = part_size + 1;
            else
                bufftosend = part_size;

            matrix_sendcounts[i] = bufftosend * (x + 1);
