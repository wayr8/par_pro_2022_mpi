...
TEST(MPI_Scatter, int_to_int) {
    ...
    FasterScatter(buf.data(), buf.size() / size, MPI_INT, recv.data(),
                  buf.size() / size, MPI_INT, root, MPI_COMM_WORLD);
    MPI_Scatter(buf.data(), buf.size() / size, MPI_INT, scatter.data(),
                buf.size() / size, MPI_INT, root, MPI_COMM_WORLD);
    MPI_Gather(recv.data(), recv.size(), MPI_INT, buf1.data(), recv.size(),
               MPI_INT, root, MPI_COMM_WORLD);
    MPI_Gather(scatter.data(), scatter.size(), MPI_INT, buf2.data(),
               scatter.size(), MPI_INT, root, MPI_COMM_WORLD);
...
TEST(MPI_Scatter, double_to_float) {
    ...
    FasterScatter(buf.data(), buf.size() / size, MPI_DOUBLE, recv.data(),
                  recv.size(), MPI_FLOAT, root, MPI_COMM_WORLD);
    MPI_Scatter(buf.data(), buf.size() / size, MPI_DOUBLE, scatter.data(),
                recv.size(), MPI_FLOAT, root, MPI_COMM_WORLD);

    MPI_Gather(recv.data(), recv.size(), MPI_FLOAT, buf1.data(), recv.size(),
               MPI_FLOAT, root, MPI_COMM_WORLD);
    MPI_Gather(scatter.data(), scatter.size(), MPI_FLOAT, buf2.data(),
               scatter.size(), MPI_FLOAT, root, MPI_COMM_WORLD);
    if (rank == 0) {
        ASSERT_EQ(buf1, buf2);
    }
}
TEST(MPI_Scatter, throw_when_error_count_and_root) {
    ...
    ASSERT_ANY_THROW(FasterScatter(buf.data(), buf.size() / 3, MPI_INT,
                                   recv.data(), buf.size() / 3, MPI_INT, root,
                                   MPI_COMM_WORLD));
    ASSERT_ANY_THROW(FasterScatter(buf.data(), 0, MPI_INT, recv.data(), 0,
...
TEST(MPI_Scatter, char_to_int) {
    ...
    FasterScatter(buf.data(), buf.size() / size, MPI_CHAR, recv.data(),
                  recv.size(), MPI_INT, root, MPI_COMM_WORLD);
    MPI_Scatter(buf.data(), buf.size() / size, MPI_CHAR, scatter.data(),
                recv.size(), MPI_INT, root, MPI_COMM_WORLD);
...
TEST(MPI_Scatter, int_to_char) {
    ...
    FasterScatter(buf.data(), buf.size() / size, MPI_INT, recv.data(),
                  recv.size(), MPI_CHAR, root, MPI_COMM_WORLD);
    MPI_Scatter(buf.data(), buf.size() / size, MPI_INT, scatter.data(),
                recv.size(), MPI_CHAR, root, MPI_COMM_WORLD);

    MPI_Gather(recv.data(), recv.size(), MPI_CHAR, buf1.data(), recv.size(),
               MPI_CHAR, root, MPI_COMM_WORLD);
    MPI_Gather(scatter.data(), scatter.size(), MPI_CHAR, buf2.data(),
               scatter.size(), MPI_CHAR, root, MPI_COMM_WORLD);
    if (rank == 0) {
        ASSERT_EQ(buf1, buf2);
    }
}
