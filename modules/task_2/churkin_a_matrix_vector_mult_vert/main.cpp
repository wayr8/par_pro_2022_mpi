...
#include <vector>
#include "./matrix_vector_mult_vert.h"
...
void runTest(int m_glob, int n_glob, bool printValues = true) {
    ...
    std::vector<int> resPar = getMultParallel(globalMatrix, globalVector, m_glob, n_glob);

    if (rank == 0) {
        t2Par = MPI_Wtime();
        t1Seq = t2Par;
        std::vector<int> resSeq = getMultSequential(globalMatrix, globalVector, m_glob, n_glob);
        t2Seq = MPI_Wtime();
...
/* 2 x 3 matrix */
TEST(Matrix_On_Vector_Mult_Vert_MPI, Test_1) {
    runTest(2, 3);
}

/* 3 x 2 matrix */
TEST(Matrix_On_Vector_Mult_Vert_MPI, Test_2) {
    runTest(3, 2);
}

/* 3 x 3 matrix */
TEST(Matrix_On_Vector_Mult_Vert_MPI, Test_3) {
    runTest(3, 3);
}

/* 4 x 4 matrix */
TEST(Matrix_On_Vector_Mult_Vert_MPI, Test_4) {
    runTest(4, 4);
}

/* 10 x 10 matrix */
TEST(Matrix_On_Vector_Mult_Vert_MPI, Test_5) {
    runTest(10, 10);
}

/* 25 x 25 matrix */
TEST(Matrix_On_Vector_Mult_Vert_MPI, Test_6) {
    runTest(25, 25);
}

/* 100 x 100 matrix */
TEST(Matrix_On_Vector_Mult_Vert_MPI, Test_7) {
    runTest(100, 100, false);
}
