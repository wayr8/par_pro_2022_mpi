...
std::vector<int> getParallelOperation(Matrix<int> global_matr) {
  ...
  if (rank == 0) {
    int remained_rows = rows_number > size
                            ? rows_number % size - 1
                            : 0;  // -1 because process #0 will get one of them
    for (int proc = 1; proc < size && proc <= rows_number; proc++) {
      ...
      MPI_Send(global_matr[proc * (rows_per_process + adrow)],
               (rows_per_process + adrow) * cols_number, MPI_INT, proc, 0,
               MPI_COMM_WORLD);
    }
  }
  ...
  if (rank == 0) {
    local_vec =
        std::vector<int>(global_matr[0], global_matr[rows_per_process + adrow]);
  } else {
    MPI_Status status;
    MPI_Recv(local_vec.data(), (rows_per_process + adrow) * cols_number,
  ...
  for (int i = 0; i < rows_per_process + adrow; ++i) {
    ...
    outf << "Process " << rank << ": I found max at row " << rank + i << ": "
         << max_values[rank + i] << std::endl;
  }
