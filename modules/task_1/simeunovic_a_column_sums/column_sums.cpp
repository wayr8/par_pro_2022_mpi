...
void DataDistribution(int ProcSize, int ProcRank, std::vector<int>& pSendInd, std::vector<int>& pSendNum, const std::vector<int>& matrix, std::vector<int>& pProcColumns, int row_num, int column_num)
{
	...
	MPI_Scatterv(matrix_transport.data(), pSendNum.data(), pSendInd.data(), MPI_INT, pProcColumns.data(), pSendNum[ProcRank], MPI_INT, 0, MPI_COMM_WORLD);
...
void ColumnSumsParallel(int ProcRank,int ProcSize,std::vector<int>& pSendInd,std::vector<int>& pSendNum, std::vector<int>& result, const std::vector<int>& pProcColumns, std::vector<int>& pProcResults,int row_num, int column_num, int ColumnNum)
{
	...
	MPI_Gatherv(pProcResults.data(), pSendNum[ProcRank], MPI_INT, result.data(), pSendNum.data(), pSendInd.data(), MPI_INT, 0, MPI_COMM_WORLD);
}
...
void DoWork(std::vector<int>& a,std::vector<int>& b)
{
	...
	ProcessInitialization(ProcRank, ProcSize, matrix, result, pProcColumns, pProcResults, row_num, column_num, ColumnNum);
	DataDistribution(ProcSize, ProcRank, pSendInd, pSendNum, matrix, pProcColumns, row_num, column_num);
	ColumnSumsParallel(ProcRank,ProcSize,pSendInd, pSendNum, result, pProcColumns, pProcResults,row_num, column_num, ColumnNum);
	if (ProcRank == 0)
	{
