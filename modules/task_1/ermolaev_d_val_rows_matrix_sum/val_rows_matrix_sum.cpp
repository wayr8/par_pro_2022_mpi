...
void getSequentialOperation(int* matrix, int* result, int x, int y)
{
    ...
    for (int j = 0; j < y; j++)
    {
        for (int i = 0; i < x; i++)
        {
            result[j] += matrix[x * j + i]; //Правильная индексация
        }
    }
    /*std::cout << "\nSEQUENTIAL____________________________________________\n";
    for (int j = 0; j < y; j++) {
        std::cout << result[j] << "\t";
    }
    std::cout << "\n";
...
void getParallelOperation(int* matrix, int* result, int x, int y)
{
    ...
    if (id == 0)
    {
        ...
        {
            MPI_Send(matrix + x * i * data_in_process, x * data_in_process, MPI_INT, i, 0, MPI_COMM_WORLD);
        }
        for (int i = 1; i < iteration; ++i)
        {
            MPI_Recv(result + i * data_in_process, data_in_process, MPI_INT, i, 0, MPI_COMM_WORLD, &status);
        }

        for (int i = 0; i < data_in_process; ++i)
        {

            result[i] = (std::accumulate(matrix + x * i, matrix + x * (i + 1), result[i]));
        }
    ...
    else if (id < y)
    {
        ...
        MPI_Recv(local_matrix, x * data_in_process, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
        for (int i = 0; i < data_in_process; ++i)
        {

            local_result[i] = (std::accumulate(local_matrix + x * i, local_matrix + x * (i + 1), local_result[i]));
        }
